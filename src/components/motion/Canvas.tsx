...
const CanvasComponent = ({ 
  ...
  const avoidanceSpheresRef = useRef<THREE.Mesh[]>([]);
      ...
      clone.traverse((child: any) => {
        if (child.isMesh && child.material) {
          ...
          if (!isStandard && !isBasic) {
            ...
            // MeshStandardMaterial automatically handles skinning based on geometry
            child.material = new THREE.MeshStandardMaterial({ color, metalness: 0.1, roughness: 0.9 });
          }
        }
      });
    ...
    async function setupModels() {
      ...
      if (bvhFile && targetModels.length > 0) {
        try {
          ...
          // Remove existing avoidance spheres
          avoidanceSpheresRef.current.forEach(sphere => {
            scene.remove(sphere);
            sphere.geometry.dispose();
            (sphere.material as THREE.Material).dispose();
          });
          avoidanceSpheresRef.current = [];

          // Add MaskControl avoidance point markers if this is a MaskControl BVH
          const isMaskControlFile = bvhFile.includes('avoidance_output.bvh');
          if (isMaskControlFile) {
            ...
            console.log('🎯 Adding MaskControl avoidance point markers');
            avoidancePoints.forEach((point, index) => {
              const geometry = new THREE.SphereGeometry(0.6, 32, 32); // Bigger spheres (0.8 radius)
              ...
              sphere.name = `avoidance-point-${index}`;
              scene.add(sphere);
              avoidanceSpheresRef.current.push(sphere);
            });
          }
  ...
  return (
    <div style={{ position: "relative", height: "100%" }} className="bg-zinc-900">
