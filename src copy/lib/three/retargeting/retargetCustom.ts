...
export function retargetCustomModel(source: any, targetModel: any): RetargetResult | null {
  ...
  console.log('🔍 Custom - Source skeleton bones:', source.skeleton.bones.map((b: any) => b.name));
  console.log('🔍 Custom - Source clip tracks:', source.clip.tracks.length);
  console.log('🔍 Custom - Target skin skeleton bones:', targetSkin.skeleton.bones.map((b: any) => b.name).slice(0, 10), '...');
